...
%%CALCOLI FINALI ALGORITMO2023
l1 = valore_minimo_l_2023;
l2 = valore_minimo_2_2023;
l3 = valore_minimo_3_2023;

alfa=(0:0.01:1);
beta=1-alfa;
syms N;

%m_e(1)=64,Q(2)=100 fissato,lambda minimo caso 1
a_2023_1=((-l1.*Q(2).*t_se_2023.*(m_e(1).^2).*alfa)-((Q(2).*t_se_2023.*m_e(1).^2).*beta)./2-((l1.*Q(2).*sigma_se_2023.^2.*m_e(1).^2.*beta))./2).*N^2;

b_2023_1=((2.*l1.^2.*Q(2).^2.*t_se_2023.^2.*m_e(1).*alfa)-(l1.*Q(2).^2.*t_se_2023.^2.*(m_e(1)-1).*m_e(1).*beta)).*N;

c_2023_1=((-l1.^3.*Q(2).^3.*t_se_2023.^3.*alfa)+((l1.^2.*Q(2).^3.*t_se_2023.^3.*beta.*(m_e(1)-1))./2));

eq_2023_1=a_2023_1+b_2023_1+c_2023_1;

solutions_2023_1=solve(eq_2023_1==0,N);
...
%m_e=128,Q=100 fissato,lambda minimo caso 1
a_2023_2=((-l1.*Q(2).*t_se_2023.*(m_e(2).^2).*alfa)-((Q(2).*t_se_2023.*m_e(2).^2).*beta)./2-((l1.*Q(2).*sigma_se_2023.^2.*m_e(2).^2.*beta))./2)*N^2;

b_2023_2=((2.*l1.^2.*Q(2).^2.*t_se_2023.^2.*m_e(2).*alfa)-(l1.*Q(2).^2.*t_se_2023.^2.*(m_e(2)-1).*m_e(2).*beta)).*N;

c_2023_2=((-l1.^3.*Q(2).^3.*t_se_2023.^3.*alfa)+((l1.^2.*Q(2).^3.*t_se_2023.^3.*beta.*(m_e(2)-1))./2));

eq_2023_2=a_2023_2+b_2023_2+c_2023_2;

solutions_2023_2=solve(eq_2023_2==0,N);


%m_e=256,Q=100 fissato,lambda minimo caso 1
a_2023_3=((-l1.*Q(2).*t_se_2023.*(m_e(3).^2).*alfa)-((Q(2).*t_se_2023.*m_e(3).^2).*beta)./2-((l1.*Q(2).*sigma_se_2023.^2.*m_e(3).^2.*beta))./2).*N^2;

b_2023_3=((2.*l1.^2.*Q(2).^2.*t_se_2023.^2.*m_e(3).*alfa)-(l1.*Q(2).^2.*t_se_2023.^2.*(m_e(3)-1).*m_e(3).*beta)).*N;

c_2023_3=((-l1.^3.*Q(2).^3.*t_se_2023.^3.*alfa)+((l1.^2.*Q(2).^3.*t_se_2023.^3.*beta.*(m_e(3)-1))./2));

eq_2023_3=a_2023_3+b_2023_3+c_2023_3;

solutions_2023_3=solve(eq_2023_3==0,N);
